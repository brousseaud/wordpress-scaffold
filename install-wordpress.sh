...
# Variables
CONFIG_URL="https://raw.githubusercontent.com/brousseaud/wordpress-scaffold/main/configs/site-plugins.yaml"
...
for plugin in $($YQ e '.plugins.free[]' $CONFIG_FILE); do
  echo "Installing plugin: $plugin"
  $PHP_CLI $WP_CLI plugin install "$plugin" --activate
done

# Install ZIP plugins
ZIP_COUNT=$($YQ e '.plugins.zips | length' $CONFIG_FILE)
if [ "$ZIP_COUNT" -gt 0 ]; then
  for i in $(seq 0 $(($ZIP_COUNT - 1))); do
    PLUGIN_URL=$($YQ e ".plugins.zips[$i].url" $CONFIG_FILE)
    PLUGIN_NAME=$($YQ e ".plugins.zips[$i].name" $CONFIG_FILE)
